...
**当前版本：[v1.8.8](https://github.com/zhangweildlh/chrome-md-editor/releases/tag/v1.8.8)**  
**下载（Chrome 扩展）：** [chrome-md-editor-v1.8.8.zip](https://github.com/zhangweildlh/chrome-md-editor/releases/download/v1.8.8/chrome-md-editor-v1.8.8.zip)  
**下载（Windows 独立 EXE）：** [Markdown.Editor_1.8.8_portable.exe](https://github.com/zhangweildlh/chrome-md-editor/releases/download/v1.8.8/Markdown.Editor_1.8.8_portable.exe)  
**许可：** [MIT](./LICENSE)
...
- **下载**：Release `v1.8.8` 中的 `Markdown.Editor_1.8.8_portable.exe`（便携版，直接拷到任意目录双击运行）。
...
- **本地零安装构建**：EXE 完全在 GitHub 云端（`windows-latest`）用 Rust + Tauri 构建；一次打 `v1.8.8` 标签即**同时产出扩展 zip 与 EXE 两个资产**，开发者本地无需安装 Rust / Tauri / WebView2。
...
2. 关掉所有旧的编辑器标签，再新开一页。
3. 确认左上角版本徽标与 Release 一致（当前应为 **v1.8.8**）。
4. 桌面 EXE 用户：重新下载 `Markdown.Editor_1.8.8_portable.exe` 覆盖旧文件即可，无需卸载。
...
The toolbar should show the release version (currently **v1.8.8**).
...
A portable Windows build is also provided: the same web editor packaged with Tauri v2 into a green, install-free `Markdown.Editor_1.8.8_portable.exe`. Double-clicking a `.md` (after setting it as the default opener) or dragging a `.md` into the window opens it directly; each open runs as an independent instance. Saves write back to the original file. The EXE relies on the system WebView2 runtime and is built entirely in GitHub Actions (no local Rust/Tauri toolchain needed). Download from the `v1.8.8` release.
