  ...
  container.innerHTML = `
    <div class="onboarding-card onboarding-card--wide">
      ...
      <div class="onboarding-footer">
        ${
          isGuide
            ? `<p class="onboarding-hint">编辑增强（主题 / 斜杠菜单 / 块拖拽 / 视图模式 / 工作区搜索 / 高亮 / 撤销重做 / 设置菜单）的用法，点「打开示例说明书」查看「功能一览」。</p>
...
| 分类 | 功能 | 在哪用 |
|------|------|--------|
| 打开 / 保存 | 打开、保存、新建、撤销、重做 | 顶部文件栏（Ctrl+O、Ctrl+S、Ctrl+Z、Ctrl+Y） |
| 文本样式 | 加粗 / 斜体 / 删除线 / 行内代码 / 居中 / 颜色 / 字号 / 引用 / 代码块 / 链接 / 表格 / 水平线 / 图片 | 顶部样式栏 |
| 标题 / 列表 | H1 / H2 / H3 / 有序列表 / 无序列表 | 顶部「标题 / 列表」菜单 |
| 视图 | 分屏 / 仅编辑 / 仅预览；日常 / 专注 / 沉浸 / 全显 | 顶部「⚙ 设置」→ 视图模式；或「⊞」按钮循环切换 |
| 外观 | 33 套主题、编辑器配色 | 顶部「⚙ 设置」→ 外观（主题下拉） |
| 高亮 | 行内 mark 高亮；编辑区语法高亮 / 预览区代码着色（两个独立入口） | 样式栏「高亮(A)」；「⚙ 设置」→ 高亮方案 |
| 自动保持 | 开关 + 间隔秒数（默认 30 秒） | 顶部「⚙ 设置」→ 自动保持 |
| 增强 | 专注模式、打字机滚动、显示设置 | 顶部「⚙ 设置」→ 增强 / 显示设置 |
| 其他 | 阅读翻译（双语对照）、使用说明 | 顶部「⚙ 设置」→ 其他 |
| 同步 | 编辑区与预览区联动滚动 | 顶部「同步」（锁链图标） |
| 查找 | 查找 / 替换、工作区搜索 | 顶部「查找」(Ctrl+F) → 面板内「工作区搜索」 |
| 进阶 | Mermaid、行内 == 高亮 ==、GitHub 提示框、斜杠菜单、块拖拽、多标签 | 见上方各节与提示 |
`;
